---
title: "Visualisation for Overimputation Diagnostics"
format:
  html:
    toc: true
    page-layout: full
...
We can obtain an overimputation object with 5 multiple imputations (`m = 5`), extra missing values with proprotion 20% (`p = 0.2`) and test set ratio 20% (`test_ratio = 0.2`). Imputation method can be set to `"mixgb"` or `"mice"`, which would call `mixgb()` or `mice` in the backend. Users can also pass additional argument related to `mixgb()` or `mice()` through `overimp()`. 
