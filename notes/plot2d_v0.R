...
plotly_2num<- function(data, imp_list, x, y, color_pal = NULL,
  ...
  fig<-.plotly_scatter(fig, all_dt,x,y,point_size,alpha,color_pal)
  ...
  if(!is.null(marginal_x)){
    if(marginal_x=="box+rug"){
      ...
      fig<-.plotly_rug_x(fig, all_dt,x,color_pal)
    }
  }
  ...
  if(!is.null(marginal_y)){
    if(marginal_y=="box+rug"){
      ...
      fig<-.plotly_rug_y(fig, all_dt,y,color_pal)
    }
  }
  ...
  if(is.null(marginal_x) & is.null(marginal_y)){
    ...
    fig <- fig |> layout(
      title = paste("Observed vs multiply-imputed values:", y, "vs", x),
      xaxis = list(domain = c(0, 0.7), title = x),
      yaxis = list(domain = c(0, 0.8), title = y),
      xaxis2 = list(domain = c(0.7, 1), showticklabels = FALSE),
      yaxis2 = list(domain = c(0.8, 1), showticklabels = FALSE),
