...
.plotly_box_x<-function(fig,all_dt, x,color_pal){
  for (group in levels(all_dt$Group)) {
    df_group <- all_dt |> filter(Group == group)
    fig <- fig |> add_trace(
      x = df_group[[x]],
...
.plotly_box_y<-function(fig,all_dt, y,color_pal){
  for (group in levels(all_dt$Group)) {
    df_group <- all_dt |> filter(Group == group)
    fig <- fig |> add_trace(
      y = df_group[[y]],
...
# rug---------------------------------------------------------------------
.plotly_rug_x <- function(fig, all_dt, x, color_pal,mx_offsets) {
  for (group in levels(all_dt$Group)) {
...
.plotly_rug_y <- function(fig, all_dt, y, color_pal,my_offsets) {
  for (group in levels(all_dt$Group)) {
    df_group <- all_dt |> dplyr::filter(Group == group)
...
.plotly_hist_x<-function(fig,all_dt, x,color_pal,nbins){
...
.plotly_hist_y<-function(fig,all_dt, y,color_pal, nbins){
