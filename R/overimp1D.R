...
overimp1D_cv_num<-function(plot_data, x, comb_title, point_size, xlim, ylim, train_color_pal, test_color_pal){
  ...
  .transform_data<-function(data, x){
    mt_data<-data %>% filter(Group=="Masked true") %>%
      select(row_index, `Masked true` = all_of(x))

    join_data<-data %>% filter(Group!="Masked true") %>%
      left_join(mt_data, by="row_index")

    line_data <- join_data%>%
      group_by(.data[["Masked true"]]) %>%
      summarize(ymin = min(.data[[x]]), ymax = max(.data[[x]]), .groups = "drop")
    max_val <- max(data[[x]])
    min_val <- min(data[[x]])
  ...
  train_plot<- ggplot(train_data$join_data, aes(x=.data[["Masked true"]], y=.data[[x]], color = Group)) +
    geom_point(size=0.5)+
    geom_segment(data = train_data$line_data, aes(x = .data[["Masked true"]], xend = .data[["Masked true"]],
                                                  y = ymin, yend = ymax),
  ...
  if(!is.null(plot_data$test)) {
    ...
    test_plot<- ggplot(test_data$join_data, aes(x=.data[["Masked true"]], y=.data[[x]], color = Group)) +
      geom_point(size=0.5)+
      geom_segment(data = test_data$line_data, aes(x = .data[["Masked true"]], xend = .data[["Masked true"]],
                                                   y = ymin, yend = ymax),
                   inherit.aes = FALSE, color = "gray40", linetype = "solid", linewidth = 0.1) +
      geom_abline(slope = 1, intercept = 0, linetype = "solid", color = "gray20")+
      #coord_fixed(xlim = c(min_val, max_val), ylim = c(min_val, max_val))+
...
overimp1D_cv_fac<-function(plot_data, x, comb_title, point_size, xlim, ylim,train_color_pal, test_color_pal,stack_y, diag_color,seed){
  ...
  if(stack_y){
    ...
    if(!is.null(diag_color)){
      ...
      bg <- expand.grid(
        ...
        mutate(
          ...
          ymin = y - 0.5,
          ymax = y + 0.5,
          bg_color = ifelse(x == y, diag_color, "transparent")
        )
    ...
    train_data <-train_data %>%
      mutate(y_offset = -0.4+y+(row_id-1)*0.8/max_n)


    train_plot<-train_plot+
      geom_point(
        data = train_data, aes(x=x_num+group_offset, y=y_offset, color = Group),
        size = point_size
      )+
      scale_x_continuous(
    ...
    train_plot<-ggplot(train_data, aes(x=x_num+group_offset, y=.data[[x]], color = Group)) +
      geom_point(
  ...
  if(!is.null(plot_data$test)) {
    ...
    if(stack_y){
      ...
      test_plot<-test_plot+
        geom_point(
          data = test_data, aes(x=x_num+group_offset, y=y_offset, color = Group),
          size = point_size
        )+
        scale_x_continuous(
          breaks = 1:length(x_labels), # The integer positions
      ...
      test_plot<-ggplot(test_data, aes(x=x_num+group_offset, y=.data[[x]], color = Group)) +
        geom_point(
          position = position_jitter(width=0,height =0.3, seed = seed),
          size = point_size
        )+
