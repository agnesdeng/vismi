...
#' @param imputation.lists A list of lists.... \code{m} imputed datasets returned by the \code{mixgb} imputer, or other package.
#' @param methods A vector of length \code{m} containing the names of the imputation methods used to generate the \code{m} imputed datasets. Default: NULL (use "Method 1", "Method 2", etc.)
...
plot_density_facet <- function(imputation.lists, methods, var.name, original.data, true.data = NULL, color.pal = NULL,xlim = NULL) {
  ...
  P<-ggplot(data = all.dt, aes(x = .data[[var.name]])) +
    ...
    geom_density(linewidth = 1, alpha = 0.2, aes(color = m.set,fill=m.set)) +
...
#' Overlay density plots for multiply imputed values for two numeric variables
#' @description Plot overlay density curves of observed values versus m sets of imputed values for a specified numeric variable using \pkg{ggplot2}.
#' @param imputation.list A list of lists....of \code{m} imputed datasets returned by the \code{mixgb} imputer, or other package.
#' @param methods A list of methods?
#' @param var.x The name of the variable on the x axis.
#' @param var.y The name of the variable on the y axis.
#' @param original.data The original data with missing values.
#' @param true.data The true data without missing values. This is generally unknown in practice. If the true data is known (e.g., in cases where it is generated by simulation), it can be specified in this argument. The output will then have an extra panel called \code{MaskedTrue}, which shows values originally observed but intentionally made missing.
#' @param color.pal A vector of hex color codes for the observed and m sets of imputed values panels. The vector should be of length \code{m+1}. Default: NULL (use "gray40" for the observed panel, use ggplot2 default colors for other panels.)
#' @param xlim the limit of the x axis
