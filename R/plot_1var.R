...
#' @description High-level function to plot observed values versus m sets of imputed values for a specified variable
...
#' @param num.plot The type of plot for numeric variables. Options are "hist" (default), "box", or "density".
#' @param int.plot The type of plot for integer variables. Options are "bar" (default), "hist", "box", or "density".
...
plot1D<-function(imputation.list, var.name, original.data, true.data = NULL, color.pal = NULL,
  ...
  if (Types[var.name] == "numeric") {
   if(num.plot=="hist"){
    plot_hist(imputation.list = imputation.list, var.name = var.name,
              original.data = original.data, true.data = true.data, color.pal = color.pal)
   }else if(num.plot=="box"){
     plot_box(imputation.list = imputation.list, var.name = var.name,
              original.data = original.data, true.data = true.data, color.pal = color.pal)
   }else if(num.plot=="density"){
    plot_density(imputation.list = imputation.list, var.name = var.name,
              original.data = original.data, true.data = true.data, color.pal = color.pal)
   }else{
     stop("The value specified in `num.plot` is not valid. Please use `hist`, `box`, or `density`.")
   }
  }else if(Types[var.name] == "integer"){
    if(int.plot=="bar"){
      plot_bar(imputation.list = imputation.list, var.name = var.name,
               original.data = original.data, true.data = true.data, color.pal = color.pal)
    }else if(int.plot=="hist"){
      plot_hist(imputation.list = imputation.list, var.name = var.name,
                original.data = original.data, true.data = true.data, color.pal = color.pal)
    }else if(int.plot=="box"){
      plot_box(imputation.list = imputation.list, var.name = var.name,
               original.data = original.data, true.data = true.data, color.pal = color.pal)
      ...
      stop("The value specified in `int.plot` is not valid. Please use `bar`, hist`, `box`, or `density`.")
    }
  }else{
    plot_bar(imputation.list = imputation.list, var.name = var.name,
             original.data = original.data, true.data = true.data, color.pal = color.pal)
...
plot_density <- function(imputation.list, var.name, original.data, true.data = NULL, color.pal = NULL) {
  ...
  ggplot(data = all.dt, aes(x = .data[[var.name]])) +
    ...
    geom_density(linewidth = 1, alpha = 0.3, aes(color = m.set)) +
...
plot_hist <- function(imputation.list, var.name, original.data, true.data = NULL, color.pal = NULL) {
  ...
  if(Types[var.name]=="integer"){
    ggplot(data = all.dt, aes(x = .data[[var.name]])) +
      # geom_histogram(alpha = 0.5, aes(fill = m.set, color = m.set, y = ..density..), breaks = breaks) +
      geom_histogram(binwidth = 1,alpha = 0.5, aes(fill = m.set, color = m.set, y = after_stat(density))) +
...
plot_box <- function(imputation.list, var.name, original.data, true.data = NULL, color.pal = NULL) {
  ...
  gp +
    ...
    labs(x = "count", title = "Box plots with data points", subtitle = paste(" imputed sets for variable: ", var.name)) +
    scale_color_manual(values = color.pal) +
    scale_fill_manual(values = color.pal) +
